...
const UserInfo = () => {
	...
	const viewObserver: UserInfoView = {
		setIsLoading,
		setIsFollower,
		setFolloweeCount,
		setFollowerCount,
	...
	if (!presenter.current) {
		presenter.current = new UserInfoPresenter(viewObserver);
	}
	...
	useEffect(() => {
		...
		presenter.current?.setNumFollowers(authToken!, displayedUser!);
		presenter.current?.setNumFollowees(authToken!, displayedUser!);
	}, [displayedUser]);


	const switchToLoggedInUser = (event: React.MouseEvent): void => {
		event.preventDefault();
		presenter.current?.switchToLoggedInUser(currentUser!, location);
	};

	const followDisplayedUser = async (event: React.MouseEvent): Promise<void> => {
		event.preventDefault();
		presenter.current?.followDisplayedUser(authToken!, displayedUser!);
	};

	const unfollowDisplayedUser = async (event: React.MouseEvent): Promise<void> => {
		event.preventDefault();
		presenter.current?.unfollowDisplayedUser(authToken!, displayedUser!);
	};
